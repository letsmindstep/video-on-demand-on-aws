...
export class VideoOnDemand extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const workflowTrigger = new cdk.CfnParameter(this, "WorkflowTrigger", {
      type: "String",
      description:
        "How the workflow will be triggered (source video upload to S3 or source metadata file upload)",
      default: "VideoFile",
    ...
    const frameCapture = new cdk.CfnParameter(this, "FrameCapture", {
      type: "String",
      description:
        "If enabled, frame capture is added to the job submitted to MediaConvert",
      default: "No",
      allowedValues: ["Yes", "No"],
    });
    const enableMediaPackage = new cdk.CfnParameter(
      this,
      "EnableMediaPackage",
      {
        type: "String",
        description:
          "If enabled, MediaPackage VOD will be included in the workflow",
        default: "No",
        allowedValues: ["Yes", "No"],
      }
    );
    const enableSns = new cdk.CfnParameter(this, "EnableSns", {
      type: "String",
      description:
        "Enable Ingest and Publish email notifications, error messages are not affected by this parameter.",
      default: "Yes",
      allowedValues: ["Yes", "No"],
    });
    const enableSqs = new cdk.CfnParameter(this, "EnableSqs", {
      type: "String",
      description:
        "Publish the workflow results to an SQS queue to injest upstream",
      default: "Yes",
      allowedValues: ["Yes", "No"],
    });
    const acceleratedTranscoding = new cdk.CfnParameter(
      this,
     ...
     */
    new cdk.CfnMapping(this, "AnonymizedData", {
      // NOSONAR
      mapping: {
        SendAnonymizedData: {
          Data: "Yes",
     ...
     * Conditions
     */
    const conditionEnableMediaPackage = new cdk.CfnCondition(
      this,
      "EnableMediaPackageCondition",
      {
        expression: cdk.Fn.conditionEquals(
          enableMediaPackage.valueAsString,
          "Yes"
        ),
      }
    );
    const conditionFrameCapture = new cdk.CfnCondition(
      this,
    ...
    const conditionEnableSns = new cdk.CfnCondition(
      this,
      "EnableSnsCondition",
      {
    ...
    const conditionEnableSqs = new cdk.CfnCondition(
      this,
      "EnableSqsCondition",
      {
    ...
    const cfnDestination = destination.node.findChild(
      "Resource"
    ) as s3.CfnBucket;
    cfnDestination.cfnOptions.deletionPolicy = cdk.CfnDeletionPolicy.RETAIN;
    cfnDestination.cfnOptions.updateReplacePolicy =
    ...
    NagSuppressions.addResourceSuppressions(
      ...
      [
        ...
        },
        {
          ...
          reason:
            "Use case does not warrant CloudFront integration with AWS WAF",
        },
        {
    ...
    //cfn_nag
    const cfnCustomResourceRole = customResourceRole.node.findChild(
      "Resource"
    ) as iam.CfnRole;
      ...
      cfn_nag: {
        rules_to_suppress: [
          ...
          },
          {
            id: "W76",
            reason:
              "High complexity due to number of policy statements needed for creating all custom resources",
          },
        ],
      },
    };
    ...
    //cfn_nag
    const cfnCustomResourceLambda = customResourceLambda.node.findChild(
      "Resource"
      ...
      cfn_nag: {
        rules_to_suppress: [
          ...
          },
          {
            id: "W89",
            reason:
              "This CustomResource does not need to be deployed inside a VPC",
          },
          {
            id: "W92",
            reason:
     ...
     */
    const mediaConvertEndpoint = new cdk.CustomResource(
      this,
     ...
     */
    const mediaConvertTemplates = new cdk.CustomResource(
      this,
      "MediaConvertTemplates",
      {
        ...
        properties: {
          ...
          EnableMediaPackage: cdk.Fn.conditionIf(
    ...
    //cfn_nag
    const cfnMediaConvertRole = mediaConvertRole.node.findChild(
      "Resource"
    ) as iam.CfnRole;
    ...
    const mediaPackageVodPolicy = new iam.Policy(
      this,
    ...
    //cfn_nag
    const cfnMediaPackageVodRole = mediaPackageVodRole.node.findChild(
      "Resource"
    ) as iam.CfnRole;
    ...
    const snsTopic = new sns.Topic(this, "SnsTopic", {
      ...
      masterKey: kms.Alias.fromAliasName(
        this,
        "AwsManagedAwsKmsForAmazonSns",
        "alias/aws/sns"
      ),
    });
    snsTopic.addSubscription(
      new subscriptions.EmailSubscription(adminEmail.valueAsString)
    );
    ...
    const cfnDynamoDB = dynamoDBTable.node.findChild(
      "Resource"
      ...
      cfn_nag: {
        rules_to_suppress: [
          ...
          },
          {
            id: "W74",
            reason:
              "The DynamoDB table is configured to use the default encryption",
          },
        ],
      },
    };
    ...
    //cfn_nag
    const cfnErrorHandlerRole = errorHandlerRole.node.findChild(
      "Resource"
    ) as iam.CfnRole;
    ...
    //cfn_nag
    const cfnErrorHandlerLambda = errorHandlerLambda.node.findChild(
      "Resource"
    ) as lambda.CfnFunction;
    ...
    //cfn_nag
    const cfnInputValidateRole = inputValidateRole.node.findChild(
      "Resource"
    ) as iam.CfnRole;
    ...
    const inputValidateLambda = new lambda.Function(
      this,
      "InputValidateLambda",
      {
        runtime: lambda.Runtime.NODEJS_18_X,
        ...
        environment: {
          ...
          FrameCapture: `${cdk.Fn.conditionIf(
            conditionFrameCapture.logicalId,
            "true",
            "false"
          )}`,
          ...
          EnableSns: `${cdk.Fn.conditionIf(
            conditionEnableSns.logicalId,
            "true",
            "false"
          )}`,
          EnableSqs: `${cdk.Fn.conditionIf(
            conditionEnableSqs.logicalId,
            "true",
            "false"
          )}`,
    ...
    //cfn_nag
    const cfnInputValidateLambda = inputValidateLambda.node.findChild(
      "Resource"
    ) as lambda.CfnFunction;
      ...
      cfn_nag: {
        rules_to_suppress: [
          ...
          },
          {
            id: "W92",
            reason:
              "Lambda do not need ReservedConcurrentExecutions in this case",
          },
          {
            id: "W58",
            reason: "Invalid warning: function has access to cloudwatch",
    ...
    //cfn_nag
    const cfnMediaInfoRole = mediaInfoRole.node.findChild(
      "Resource"
    ) as iam.CfnRole;
    ...
    //cfn_nag
    const cfnMediaInfoLambda = mediaInfoLambda.node.findChild(
      "Resource"
    ) as lambda.CfnFunction;
    ...
    //cfn_nag
    const cfnDynamoUpdateRole = dynamoUpdateRole.node.findChild(
      "Resource"
    ) as iam.CfnRole;
    ...
    //cfn_nag
    const cfnDynamoUpdateLambda = dynamoUpdateLambda.node.findChild(
      "Resource"
    ) as lambda.CfnFunction;
    ...
    //cfn_nag
    const cfnProfilerRole = profilerRole.node.findChild(
      "Resource"
    ) as iam.CfnRole;
    ...
    //cfn_nag
    const cfnProfilerLambda = profilerLambda.node.findChild(
      "Resource"
    ) as lambda.CfnFunction;
    ...
    //cfn_nag
    const cfnEncodeLambda = encodeLambda.node.findChild(
      "Resource"
    ) as lambda.CfnFunction;
    ...
    //cfn_nag
    const cfnOutputValidateRole = outputValidateRole.node.findChild(
      "Resource"
    ) as iam.CfnRole;
    ...
    const outputValidateLambda = new lambda.Function(
      this,
    ...
    //cfn_nag
    const cfnOutputValidateLambda = outputValidateLambda.node.findChild(
      "Resource"
    ) as lambda.CfnFunction;
    ...
    //cfn_nag
    const cfnArchiveSourceRole = archiveSourceRole.node.findChild(
      "Resource"
    ) as iam.CfnRole;
    ...
    const archiveSourceLambda = new lambda.Function(
      this,
      "ArchiveSourceLambda",
      {
        runtime: lambda.Runtime.NODEJS_18_X,
    ...
    //cfn_nag
    const cfnArchiveSourceLambda = archiveSourceLambda.node.findChild(
      "Resource"
    ) as lambda.CfnFunction;
    ...
    //cfn_nag
    const cfnSqsSendMessageRole = sqsSendMessageRole.node.findChild(
      "Resource"
    ) as iam.CfnRole;
    ...
    const sqsSendMessageLambda = new lambda.Function(
      this,
      "SqsSendMessageLambda",
      {
        runtime: lambda.Runtime.NODEJS_18_X,
        handler: "index.handler",
    ...
    //cfn_nag
    const cfnSqsSendMessageLambda = sqsSendMessageLambda.node.findChild(
      "Resource"
    ) as lambda.CfnFunction;
    ...
    const snsNotificationPolicy = new iam.Policy(
      this,
    ...
    //cfn_nag
    const cfnSnsNotificationRole = snsNotificationRole.node.findChild(
      "Resource"
    ) as iam.CfnRole;
    ...
    //cfn_nag
    const cfnSnsNotificationLambda = snsNotificationLambda.node.findChild(
      "Resource"
    ) as lambda.CfnFunction;
     ...
     */
    const mediaPackageAssetsRole = new iam.Role(
      this,
    ...
    const mediaPackageAssetsPolicy = new iam.Policy(
      this,
    ...
    //cfn_nag
    const cfnMediaPackageAssetsRole = mediaPackageAssetsRole.node.findChild(
      "Resource"
    ) as iam.CfnRole;
    ...
    const cfnMediaPackageAssetsPolicy = mediaPackageAssetsPolicy.node.findChild(
      "Resource"
    ) as iam.CfnPolicy;
    ...
    const mediaPackageAssetsLambda = new lambda.Function(
      this,
      "MediaPackageAssetsLambda",
      {
        runtime: lambda.Runtime.NODEJS_18_X,
        handler: "index.handler",
    ...
    //cfn_nag
    const cfnMediaPackageAssetsLambda = mediaPackageAssetsLambda.node.findChild(
      "Resource"
    ) as lambda.CfnFunction;
    ...
    //cfn_nag
    const cfnStepFunctionsRole = stepFunctionsRole.node.findChild(
      "Resource"
    ) as iam.CfnRole;
    ...
    const stepFunctionsLambda = new lambda.Function(
      this,
      "StepFunctionsLambda",
      {
        runtime: lambda.Runtime.NODEJS_18_X,
        ...
        description:
          "Creates a unique identifer (GUID) and executes the Ingest StateMachine",
        environment: {
          SOLUTION_IDENTIFIER: `AwsSolution/${solutionId}/%%VERSION%%`,
          AWS_NODEJS_CONNECTION_REUSE_ENABLED: "1",
    ...
    //cfn_nag
    const cfnStepFunctionsLambda = stepFunctionsLambda.node.findChild(
      "Resource"
    ) as lambda.CfnFunction;
     ...
     */
    const stepFunctionsServiceRole = new iam.Role(
      this,
    ...
    const stepFunctionsServicePolicy = new iam.Policy(
      this,
    ...
    //cfn_nag
    const cfnStepFunctionsServiceRole = stepFunctionsServiceRole.node.findChild(
      "Resource"
    ) as iam.CfnRole;
    ...
    const dynamodbUpdateTaskIngest = new tasks.LambdaInvoke(
      this,
    ...
    const dynamodbUpdateTaskProcess = new tasks.LambdaInvoke(
      this,
    ...
    const dynamodbUpdateTaskPublish = new tasks.LambdaInvoke(
      this,
    ...
    const snsNotificationTaskIngest = new tasks.LambdaInvoke(
      this,
    ...
    const snsNotificationTaskPublish = new tasks.LambdaInvoke(
      this,
    ...
    const outputValidateTask = new tasks.LambdaInvoke(
      this,
    ...
    const mediaPackageAssetsTask = new tasks.LambdaInvoke(
      this,
      "MediaPackage Assets",
      {
    ...
    const sqsSendMessageTask = new tasks.LambdaInvoke(
      this,
      "SQS Send Message",
      {
    ...
    const ingestWorkflowDefinition = inputValidateTask
      ...
      .next(
        new sfn.Choice(this, "SNS Choice (Ingest)")
          .when(
            sfn.Condition.booleanEquals("$.enableSns", true),
            snsNotificationTaskIngest
          )
    ...
    NagSuppressions.addResourceSuppressions(ingestWorkflow, [
      {
        id: "AwsSolutions-SF1",
        reason:
          "Logging handled by DynamoDB Update step and Error Handler lambda",
      },
      {
        id: "AwsSolutions-SF2",
    ...
    const processWorkflowDefinition = profilerTask
      .next(
        new sfn.Choice(this, "Encoding Profile Check")
          .when(
            sfn.Condition.booleanEquals("$.isCustomTemplate", true),
            new sfn.Pass(this, "Custom jobTemplate")
          )
          .when(
            sfn.Condition.and(
              sfn.Condition.numberEquals("$.resolution", 2160),
              sfn.Condition.stringEquals("$.orientation", "landscape")
            ),
            new sfn.Pass(this, "jobTemplate 2160p landscape")
          )
          .when(
            sfn.Condition.and(
              sfn.Condition.numberEquals("$.resolution", 2160),
              sfn.Condition.stringEquals("$.orientation", "portrait")
            ),
            new sfn.Pass(this, "jobTemplate 2160p portrait")
          )
          .when(
            sfn.Condition.and(
              sfn.Condition.numberEquals("$.resolution", 1080),
              sfn.Condition.stringEquals("$.orientation", "landscape")
            ),
            new sfn.Pass(this, "jobTemplate 1080p landscape")
          )
          .when(
            sfn.Condition.and(
              sfn.Condition.numberEquals("$.resolution", 1080),
            ...
            new sfn.Pass(this, "jobTemplate 1080p portrait")
          )
          .when(
            sfn.Condition.and(
              sfn.Condition.numberEquals("$.resolution", 720),
              sfn.Condition.stringEquals("$.orientation", "landscape")
            ),
            new sfn.Pass(this, "jobTemplate 720p landscape")
          )
          .when(
            sfn.Condition.and(
              sfn.Condition.numberEquals("$.resolution", 720),
            ...
            new sfn.Pass(this, "jobTemplate 720p portrait")
          )
          .otherwise(new sfn.Fail(this, "Invalid Encoding Profile"))
          .afterwards()
      )
      .next(
        new sfn.Choice(this, "Accelerated Transcoding Check")
          .when(
            sfn.Condition.stringEquals("$.acceleratedTranscoding", "ENABLED"),
            new sfn.Pass(this, "Enabled")
          )
          .when(
            sfn.Condition.stringEquals("$.acceleratedTranscoding", "PREFERRED"),
            new sfn.Pass(this, "Preferred")
          )
          .when(
            sfn.Condition.stringEquals("$.acceleratedTranscoding", "DISABLED"),
            new sfn.Pass(this, "Disabled")
          )
          .otherwise(
            new sfn.Fail(this, "Invalid Accelerated Transcoding Option")
          )
          .afterwards()
      )
      .next(
        new sfn.Choice(this, "Frame Capture Check")
          .when(
            sfn.Condition.booleanEquals("$.frameCapture", true),
            new sfn.Pass(this, "Frame Capture")
          )
          .when(
            sfn.Condition.booleanEquals("$.frameCapture", false),
            new sfn.Pass(this, "No Frame Capture")
          )
          .otherwise(new sfn.Fail(this, "Invalid Frame Capture Option"))
          .afterwards()
      )
      .next(encodeTask)
    ...
    sqsChoice
      .when(
        sfn.Condition.booleanEquals("$.enableSqs", true),
        sqsSendMessageTask
      )
      .otherwise(snsChoicePublish);

    const dynamoChain = sfn.Chain.start(dynamodbUpdateTaskPublish).next(
      sqsChoice
    );
    ...
    mediaPackageChoice
      .when(
        sfn.Condition.booleanEquals("$.enableMediaPackage", true),
        mediaPackageAssetsTask
      )
    ...
    const archiveSourceChoice = new sfn.Choice(this, "Archive Source Choice")
      .when(
        sfn.Condition.stringEquals("$.archiveSource", "GLACIER"),
        archiveTask
      )
      .when(
        sfn.Condition.stringEquals("$.archiveSource", "DEEP_ARCHIVE"),
        deepArchiveTask
      )
      .otherwise(mediaPackageChoice);

    const publishWorkflowDefinition =
      outputValidateTask.next(archiveSourceChoice);
    ...
    //cfn_nag
    const stateMachineDefaultPolicy = stepFunctionsServiceRole.node.findChild(
      "DefaultPolicy"
    ) as iam.Policy;
    const cfnDefaultPolicy = stateMachineDefaultPolicy.node
    ...
    NagSuppressions.addResourceSuppressions(cfnDefaultPolicy, [
      {
        id: "AwsSolutions-IAM5",
        reason:
          "Statements added to default policy by aws-stepfunctions.StateMachine class",
      },
    ]);
     ...
     */
    new cdk.CustomResource(this, "AnonymizedMetric", {
      // NOSONAR
      serviceToken: customResourceLambda.functionArn,
      properties: {
        ...
        SendAnonymizedMetric: cdk.Fn.findInMap(
          "AnonymizedData",
          "SendAnonymizedData",
          "Data"
        ),
    ...
    new cdk.CfnOutput(this, "AnonymizedMetricUUID", {
      // NOSONAR
